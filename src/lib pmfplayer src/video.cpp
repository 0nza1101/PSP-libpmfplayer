...
static unsigned int __attribute__((aligned(64)))  DisplayListHB[128 * 1024 * 4];
...
int RenderFrame(int width, int height, void* Buffer)
{
	if(init==0)
	{
	sceGuStart(GU_DIRECT, DisplayListHB);
	...
	else sceGuStart(GU_DIRECT, DisplayListHB);
...
init = 0;
	...
	if (m_PmfScaling) {
		Video.m_iWidth				= m_MovieWidth;
		Video.m_iHeight				= m_MovieHeight;
	} else {
		Video.m_iWidth				= 480;
		Video.m_iHeight				= 272;
	}
	...
	//Le bug se trouve dans le code suivant jusqu'au //
	sceGuInit();
